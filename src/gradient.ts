...
(() => {
 ...
 function paint(now){
   ...
   waves=waves.filter(w=>now-w.t<4600);
   ...
   if(now-lastPaint>40){
     ...
     for(let y=0;y<height;y++)for(let x=0;x<width;x++){
       ...
       for(const w of waves){
         ...
         const envelope=Math.exp(-front*front*28)*Math.exp(-age*.85)*Math.min(age*6,1)*w.strength;
 ...
 window.addEventListener('pointermove',e=>{
   ...
   waves.push({x,y,t:now,strength:Math.min(1,.4+distance*6),vx:previous?(x-previous.x)*aspect:0,vy:previous?y-previous.y:0});if(waves.length>9)waves.shift();
